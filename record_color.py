...
if __name__ == "__main__":
    ...
    with Recorder(configuration=configuration) as recorder:
        ...
        camera.Open()
        if camera.Width.Value > 1440:
            camera.Width.Value = 1440
            camera.OffsetX.Value = 16
        else:
            camera.OffsetX.Value = 16
            camera.Width.Value = 1440
        if camera.Height.Value > 1080:
        ...
        camera.PixelFormat.Value = "RGB8"  # Change to RGB8 for color images
        ...
        def on_configuration_update(key: str, value: typing.Any):
            ...
            elif key == "balance_white_auto":
                camera.BalanceWhiteAuto.Value = value
            else:
